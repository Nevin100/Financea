...
const InvoiceCreator = () => {
  ...
  return (
    <div className="flex h-screen p-6 bg-gray-100">
      {/* Left: Invoice Form */}
      <div className="w-2/3 bg-white p-6 rounded-lg shadow-md h-[56rem] mb-5
      ">
        ...
        <div className="mt-6">
          ...
          {invoice.items.map((item, index) => (
            <div key={index} className="flex gap-4 mt-2">
              <Input value={item.name} className="w-1/3" placeholder="Item name" readOnly/>
              <Input type="number" value={item.qty} className="w-1/6" placeholder="Qty" readOnly/>
              <Input type="number" value={item.rate} className="w-1/6" placeholder="Rate" readOnly/>
              <Input type="number" value={item.total} className="w-1/6" placeholder="Total" readOnly />
            </div>
          ))}
          <Button onClick={addItem} className="md:mt-4 bg-[#6F38C9] text-white">+ Add Item</Button>
        </div>
        ...
        <Button className="mt-6">Save as Draft</Button>
        <Button className="mt-6 ml-2 bg-[#6F38C9] text-white">Send Now</Button>
      </div>

      {/* Right: Invoice Preview */}
      <div className="w-1/3 p-6">
        <h2 className="text-lg font-semibold">Preview</h2>
        <div className="bg-white p-4 rounded-lg shadow-md mt-4 h-[500px] flex items-center justify-center">
